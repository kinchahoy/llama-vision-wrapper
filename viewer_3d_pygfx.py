...
class Battle3DViewer:
    ...
    def _update_bots(self, state: Dict):
        ...
        for bot in state.get("bots", []):
            ...
            if bot_id not in self.bot_objects:
                ...
                heading_cone.local.rotation = la.quat_from_axis_angle(
                    (1, 0, 0), math.pi / 2
                )
            ...
            bot_body.local.rotation = la.quat_from_axis_angle(
    ...
    def _update_fov_display(self):
        ...
        if should_show:
            ...
            self.fov_object.local.rotation = la.quat_from_axis_angle(
                (0, 0, 1), math.radians(bot["theta"])
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python viewer_3d_pygfx.py <battle_log.json>")
        sys.exit(1)
