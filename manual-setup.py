...
_BUILDING_WITH_SETUPTOOLS = __name__ == "__main__" and any(
    name.startswith("setuptools") for name in sys.modules
)

if _BUILDING_WITH_SETUPTOOLS:
    from setuptools import setup as _setup_stub
    ...
    WRAPPER_SRC = HERE / "wrapper_src"
    GEN_SRC = WRAPPER_SRC / "gen-helper"
    ...
    def banner(msg: str) -> None:
        ...
        step_no += 1
        typer.secho(
            f"\n=== Step {step_no}: {msg} ===", fg=typer.colors.BRIGHT_BLUE, bold=True
        )
    ...
    def setup(
        backend: Backend = typer.Option(
            ...
            "-b",
            prompt=True,
            help="cuda | metal | vulkan | none | custom",
        ),
        ...
        os.chdir(HERE)
        typer.secho(
            "=== Llama-Vision-Wrapper Setup ===", fg=typer.colors.GREEN, bold=True
        )
        ...
        if not PATCH_FILE.exists():
            typer.secho("Patch file missing!", fg=typer.colors.RED)
            raise typer.Exit(1)
        ...
        if ex in {"benchmark", "cppyy"}:
            ...
            run(
                ["uv", "run", "--directory", "old/cython-version", "cython-mtmd.py"],
                dry=dry_run,
            )
